...
let init ~width:w ~height:h = let row = List.init w (fun _ -> Color.init (0.0, 0.0, 0.0)) in
...
let%test "Scenario: Writing pixels to a canvas" = let c = init ~height:10 ~width:20 in
  let red = Color.init (1.0, 0.0, 0.0) in
  let c = write_pixel c ~x_idx:2 ~y_idx:3 ~color:red in
